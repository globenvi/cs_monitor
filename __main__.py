...
from services.ServerCheckerService import ServerCheckerService # Import your service here

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')


async def main():
    bot = Bot(config.bots.bot_tokken)  # Replace with your bot token
    dp = Dispatcher()
    generate_requirements_txt()

    # Initialize the ServerCheckerService
    server_checker_service = ServerCheckerService()

    # Start the server checker in the background
    asyncio.create_task(server_checker_service.check_servers())
